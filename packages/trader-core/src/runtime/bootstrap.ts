...
export function createGateway(config: AppConfig, logger: Logger): PolymarketGateway {
  if (config.replayInputPath) {
    return new ReplayPolymarketGateway(
  ...
  return PolymarketClobGateway.fromConfig(config, logger);
}
...
export function createTradingBot(
  ...
  const gateway = createGateway(config, logger.child("gateway"));
  const executor =
